...
export default function ClassificationPage() {
  ...
  return (
    <div className="app-container" style={{ height: "100vh", overflow: "hidden" }}>
      <div className="layout-grid" style={{ gridTemplateColumns: "320px 1fr", height: "100%" }}>

        {/* ── Left Panel — branding + info ── */}
        <div className="scroll-area" style={{ display:"flex", flexDirection:"column", padding:"36px 28px", borderRight:".5px solid var(--border)", background:"var(--surface2)" }}>
        ...
        <div className="scroll-area" style={{ display:"flex", flexDirection:"column", background:"var(--bg)" }}>
          ...
          {/* ── Camera View ── */}
          {cameraActive && (
            <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20, background:"#111", minHeight: 400 }}>
              ...
              <div style={{ display:"flex", gap:16, marginTop:24, flexWrap: "wrap", justifyContent: "center" }}>
                <button
                  onClick={stopCamera}
                  style={{ padding:"12px 24px", background:"#333", color:"#fff", borderRadius:"var(--r-md)", fontFamily:"'DM Mono',monospace", fontSize:13, border:"1px solid #444", cursor:"pointer" }}
                >
                  Cancel
                </button>
                <button
                  onClick={snapPhoto}
                  style={{ padding:"12px 24px", background:"var(--teal)", color:"#fff", borderRadius:"var(--r-md)", fontFamily:"'DM Mono',monospace", fontSize:13, border:"none", cursor:"pointer" }}
                >
                  📸 Snap Photo
                </button>
              </div>
          ...
          {!imgSrc && !cameraActive && (
            <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:20, minHeight: 400 }}>
              <div style={{ width:"100%", maxWidth:560 }}>
                <div
                  ...
                  style={{
                    ...
                    borderRadius:"var(--r-xl)", padding:"3rem 1.5rem", textAlign:"center",
                    background: dragging ? "rgba(29,158,117,.04)" : "var(--surface)",
                    cursor:"pointer", transition:"border-color .2s, background .2s",
                  }}
                >
                  <div style={{ margin:"0 auto 20px", width:64, height:64, borderRadius:"50%", background:dragging?"var(--teal-l)":"var(--surface2)", border:`.5px solid ${dragging?"var(--teal)":"var(--border2)"}`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                  ...
                  <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap: "wrap" }}>
                    <button
                      ...
                      style={{ padding:"10px 24px", background:"var(--teal)", color:"#fff", borderRadius:"var(--r-md)", fontFamily:"'DM Mono',monospace", fontSize:13, border:"none", cursor:"pointer" }}
                    >
                      Choose image
                    </button>
          ...
          {imgSrc && (
            <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
              ...
              {/* Top bar */}
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 24px", borderBottom:".5px solid var(--border)", flexShrink:0, flexWrap: "wrap", gap: 10 }}>
                <div>
                ...
                <div style={{ display:"flex", gap:10, flexWrap: "wrap" }}>
                  <button onClick={startCamera} style={{ padding:"6px 12px", background:"transparent", border:".5px solid var(--border2)", borderRadius:"var(--r-md)", fontFamily:"'DM Mono',monospace", fontSize:12, color:"var(--teal)", cursor:"pointer" }}>📷 Camera</button>
                  <button onClick={() => inputRef.current?.click()} style={{ padding:"6px 12px", background:"transparent", border:".5px solid var(--border2)", borderRadius:"var(--r-md)", fontFamily:"'DM Mono',monospace", fontSize:12, color:"var(--text2)", cursor:"pointer" }}>Replace</button>
                  <button onClick={clearImage} style={{ padding:"6px 12px", background:"transparent", border:".5px solid var(--border2)", borderRadius:"var(--r-md)", fontFamily:"'DM Mono',monospace", fontSize:12, color:"var(--text2)", cursor:"pointer" }}>Remove</button>
                </div>
              ...
              <div className="layout-grid" style={{ gridTemplateColumns: "1fr 340px", gap: 0, border: "none" }}>
                <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", background:"var(--surface2)", overflow:"hidden", padding:20, minHeight: 300 }}>
                  <img src={imgSrc} alt="Preview" style={{ maxWidth:"100%", maxHeight:"100%", objectFit:"contain", display:"block", borderRadius:8, boxShadow:"0 4px 20px rgba(0,0,0,0.05)" }} />
                </div>

                <div className="scroll-area" style={{ padding:24, background:"var(--surface)", borderLeft: ".5px solid var(--border)" }}>
                  <div style={{ fontSize:10, fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:".08em", color:"var(--text3)", marginBottom:14 }}>Analysis Status</div>
